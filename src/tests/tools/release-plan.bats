#!/usr/bin/env bats
#
# Tests for agency/tools/release-plan
#
...
setup() {
    ...
    # invoked as ./agency/tools/release-plan with the test repo as CWD.
    RELEASE_PLAN_TOOL="${REPO_ROOT}/agency/tools/release-plan"
...
@test "release-plan: --version shows version" {
    cd "$TEST_REPO"
    run ./agency/tools/release-plan --version
    assert_success
...
@test "release-plan: --help shows usage" {
    cd "$TEST_REPO"
    run ./agency/tools/release-plan --help
    assert_success
    assert_output_contains "release-plan"
...
@test "release-plan: unknown arg fails" {
    cd "$TEST_REPO"
    run ./agency/tools/release-plan --bogus
    assert_failure
...
@test "release-plan: empty repo produces basic plan with no commits" {
    cd "$TEST_REPO"
    run ./agency/tools/release-plan --no-switch --no-other
    assert_success
    assert_output_contains "Release Plan"
...
@test "release-plan: classifies workstream seeds correctly" {
    cd "$TEST_REPO"
    echo "# seed" > agency/workstreams/agency/seeds/seed-test-20260408.md
    git add agency/workstreams/agency/seeds/seed-test-20260408.md
    run ./agency/tools/release-plan --no-switch
    assert_success
...
@test "release-plan: pairs tool with matching skill into feature commit" {
    cd "$TEST_REPO"
    echo "#!/bin/bash" > agency/tools/widget
    chmod +x agency/tools/widget
    mkdir -p .claude/skills/widget
    echo "# skill" > .claude/skills/widget/SKILL.md
    git add agency/tools/widget .claude/skills/widget/SKILL.md
    run ./agency/tools/release-plan --no-switch
    assert_success
    ...
    assert_output_contains "agency/tools/widget"
    assert_output_contains ".claude/skills/widget/SKILL.md"
}

@test "release-plan: tool without matching skill stays in Tools group" {
    cd "$TEST_REPO"
    echo "#!/bin/bash" > agency/tools/lonely-tool
    chmod +x agency/tools/lonely-tool
    git add agency/tools/lonely-tool
    run ./agency/tools/release-plan --no-switch
    assert_success
    # Should land in Tools group, not as a Feature
    assert_output_contains "Tools"
    assert_output_contains "agency/tools/lonely-tool"
}
...
random:
  provider: "test"
EOF
    git add agency/config/agency.yaml
    run ./agency/tools/release-plan --no-switch
    assert_success
    assert_output_contains "Config"
    assert_output_contains "agency/config/agency.yaml"
}
...
widgets:
  provider: "github"
EOF
    git add agency/tools/widget .claude/skills/widget/SKILL.md agency/config/agency.yaml
    run ./agency/tools/release-plan --no-switch
    assert_success
    assert_output_contains "Feature"
    assert_output_contains "widget"
...
@test "release-plan: --output writes to file" {
    cd "$TEST_REPO"
    run ./agency/tools/release-plan --no-switch --output plan.md
    assert_success
...
@test "release-plan: --base changes base ref comparison" {
    ...
    run ./agency/tools/release-plan --no-switch --base origin/main
    assert_success
