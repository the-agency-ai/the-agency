...
There are two audiences with two different trees. **Dual-tracking contract:** `src/` is the source-of-truth for framework contributors; `agency/` and `.claude/` are build products regenerated by `src/tools/build` and committed to the repo. Adopters never see `src/` — they get only the build products plus their own sandbox.
...
**If you're contributing to TheAgency itself**, edit files under `src/` and re-run the build. **If you're building on top of TheAgency**, use `agency init` in your own repo — you don't need `src/` at all.
