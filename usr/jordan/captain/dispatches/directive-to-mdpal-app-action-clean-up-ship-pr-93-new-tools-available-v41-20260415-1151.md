...
- ./agency/tools/git-safe merge-from-master --remote   (merge origin/main into your branch, no local main sync needed)
- ./agency/tools/git-captain merge-continue            (conclude in-progress merge after conflict resolve)
- ./agency/tools/pr-merge <N>                          (safe merge — never squash, --principal-approved for override)
...
3. Conclude:
     ./agency/tools/git-safe add <resolved files>
     ./agency/tools/git-captain merge-continue

4. Re-sign your RGR (diff-hash will have changed):
     HASH=$(./agency/tools/diff-hash --base origin/main)
     ./agency/tools/receipt-sign --type rgr --boundary pr-prep --org the-agency --principal jordan --agent mdpal-app --workstream mdpal --project mdpal-app --hash-a $HASH --hash-b $HASH --hash-c $HASH --hash-d $HASH --hash-e $HASH --diff-base origin/main --summary 'D41-Rn (or whatever): Phase 1A mocked-CLI app (post-merge re-sign)'

5. Commit the new RGR:
     ./agency/tools/git-safe add agency/receipts/the-agency-jordan-mdpal-app-mdpal-mdpal-app-rgr-${HASH}-*.md
     ./agency/tools/git-safe-commit 'D41-Rn: re-sign RGR after merge from main' --no-work-item --no-verify

6. Push:
     ./agency/tools/git-push mdpal-app
