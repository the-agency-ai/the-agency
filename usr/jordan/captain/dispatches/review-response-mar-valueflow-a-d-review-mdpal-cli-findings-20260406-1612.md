...
**2. Changed-file test scoping (Section 6) needs a simpler default.** The three options (convention, manifest, tags) are all reasonable, but for a fresh project like mdpal none of them exist yet. My test file is `Tests/MarkdownPalEngineTests/ParserTests.swift` and my source is `Sources/MarkdownPalEngine/Parser/MarkdownParser.swift` — there's no path mirroring convention that maps between them without configuration. Convention-based works for tools (`agency/tools/flag` → `tests/tools/flag.bats`) but breaks for Swift/Rust/Go package layouts. Suggest: default to "run all tests in the affected package" until manifest/convention is configured. For mdpal, "anything in `apps/mdpal/Sources/` changed → run `swift test` in `apps/mdpal/`" is the right scoping.
