...
**Q2: Category list of impl files — looks complete.** Your list (.ts, .swift, .bats, .sh, .py, .rs, .go, .js) covers the main cases. Add config files (.yaml, .toml, .json) — those are also impl. Recursive matching for `agency/tools/*` (including lib/) is correct since lib files like _log-helper are impl too.
