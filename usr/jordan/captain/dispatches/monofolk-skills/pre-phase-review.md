...
1. **PVR** — Glob `usr/*/docs/*-pvr-*.md` for Product Vision & Requirements files.
2. **A&D** — Glob `usr/*/docs/*-architecture-*.md` and `docs/plans/*-architecture-*.md` for Architecture & Design files.
...
After decisions are applied and before getting clearance, update the scoped handoff file (`.claude-handoff-<branch-slug>.md`) with:
