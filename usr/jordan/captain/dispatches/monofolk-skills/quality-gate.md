---
allowed-tools: Bash(pnpx vitest:*), Bash(tsx:*), Bash(git status:*), Bash(git diff:*), Bash(git log:*), Bash(git add:*), Bash(oxfmt:*), Bash(oxlint:*), Bash(pnpm --filter:*), Bash(pnpm lint:*), Bash(pnpm format:*), Bash(pnpm tsc:*), Read, Glob, Grep, Edit, Write, Agent, Skill
...
- Use the project's test patterns:
  - Vitest for `tools/`: `pnpx vitest run tools/__tests__/`
  - Jest for backend: `pnpm --filter backend run test`
  - Use `tsx` directly for E2E (not `pnpx tsx`)
  - Use `tmpdir()` with a single base directory per describe block
...
Run checks scoped to the changed files:

1. **Format:** `oxfmt --check <changed-files>` (or `pnpm format:check` for monorepo-wide)
2. **Lint:** `oxlint <changed-files>` (or `pnpm lint` for monorepo-wide)
3. **Typecheck:** `pnpm --filter <affected-package> run typecheck` (if applicable — requires Doppler for backend)
4. **Tests:** Run the appropriate test suite:
   - `tools/` changes → `pnpx vitest run tools/__tests__/`
   - `apps/backend/` changes → `pnpm --filter backend run test`
   - Multiple areas → run all applicable suites
...
Where:
- `{principal}` — the principal's directory name (e.g., `jordan`)
- `{project}` — the agent/project directory name (e.g., `captain`, `folio`)
...
- `{stage-hash}` — 7-character deterministic hash of the staged changes (computed by `tsx tools/stage-hash.ts`)
...
3. Compute the stage hash: run `tsx tools/stage-hash.ts` and capture the output.
...
```
usr/jordan/folio/qgr-iteration-complete-1-2-a3b4c5d-20260331-1430.md
```
