---
allowed-tools: Bash(find:*), Bash(readlink:*), Bash(ls:*), Glob
...
2. **For each symlink**, resolve the target with `readlink` and check if it exists.
...
4. **If broken symlinks found:**

   ```
   1 broken symlink found. Run /sandbox-deactivate <name> to clean up.
   ```

5. **If no symlinks:**
   ```
   No active sandbox symlinks.
   ```
