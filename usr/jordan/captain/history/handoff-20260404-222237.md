...
- Added `git add` after every write in `agency/tools/flag`
- Added SessionStart warning in `agency/hooks/session-handoff.sh` when flags exist
...
- **Files created:**
  - `agency/workstreams/iscp/CLAUDE-ISCP.md` — workstream-scoped instructions
  - `agency/workstreams/iscp/KNOWLEDGE.md` — key decisions
  - `agency/agents/iscp/agent.md` — agent class definition
...
~50 items total (5 recovered + 29 mined + ~16 new this session). Run `./agency/tools/flag list` to see full queue. Key new flags:
...
| File | Change |
|------|--------|
| `agency/tools/flag` | git-add on write, warning on failure |
| `agency/hooks/session-handoff.sh` | Flag queue detection, principal resolution fix |
| `claude/CLAUDE-THEAGENCY.md` | Addressing, scoped CLAUDE.md, provenance headers |
| `agency/workstreams/iscp/` | NEW workstream (CLAUDE-ISCP.md, KNOWLEDGE.md) |
| `agency/agents/iscp/agent.md` | NEW agent class |
