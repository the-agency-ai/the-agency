...
- **Dispatch loop is universal** — every agent runs `/loop 5m dispatch list --status unread` at session start. Documented in `agency/CLAUDE-THEAGENCY.md` (unstaged change).
...
3. **Check collaboration:** `./agency/tools/collaboration check` — monofolk has not yet responded to SPEC-PROVIDER structural thoughts dispatch (sent yesterday). Expected an RFI back.
...
- **`day33-release-1` branch exists** with `agency/CLAUDE-THEAGENCY.md` modified (dispatch-loop universal) but unstaged
