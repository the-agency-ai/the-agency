...
- **Git:** `./agency/tools/git-safe {status|log|diff|branch|show|blame|add|merge-from-master}`, `./agency/tools/git-safe-commit`. Raw `git`, `cp`, `cat`, `gh pr create` all blocked by hookify.
...
- **CWD pitfall:** `swift build` cd's into `apps/mdpal-app/`; remember to `cd` back to worktree root to invoke `./agency/tools/*`.

## Next session startup actions

1. `dispatch list` — check for captain's reply to #316 + #325 and any new traffic
2. Re-start Monitor if the session cycled: `bash ./agency/tools/dispatch-monitor --include-collab` (via Monitor tool)
