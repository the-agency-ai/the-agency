...
- **Git:** `./agency/tools/git-safe {status|log|diff|branch|show|blame|add|merge-from-master}`, `./agency/tools/git-safe-commit`. Raw `git commit`, `cat`, `cp`, `gh pr create` blocked.
...
- On resume: start `./agency/tools/dispatch-monitor` **without** `--include-collab` if an event-driven monitor is wanted.
