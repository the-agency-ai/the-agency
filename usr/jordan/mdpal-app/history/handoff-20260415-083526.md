...
- On resume: `TaskList` and restart via `./agency/tools/dispatch-monitor --include-collab` if dead. Consider dropping `--include-collab` since the collab queue is captain's scope, not mdpal-app's, and it's generating repeated wake events.
