...
1. **Build engine Diff API** (H5 from Phase 1.5 backlog). The `mdpal diff` command requires it. Suggested API:
   ...
   - Or bundle-level: `DocumentBundle.diff(_ rev1: String, _ rev2: String) -> [SectionDiff]`
2. **CreateCommand**: `mdpal create <name> [--dir <path>]` — `DocumentBundle.create(name:initialContent:at:)` already exists
3. **HistoryCommand**: `mdpal history <bundle>` — calls `bundle.listRevisions()`, emits sorted newest-first with `latest: true`
...
5. **RevisionCreateCommand**: `mdpal revision create <bundle> [--content | --stdin] [--base-revision <id>]`
...
After 2.4 ships:
- Run `/phase-complete` for Phase 2 (deep QG, principal approval REQUIRED)
- Once PR #179 merges, can ship Phase 2 work (or close PR #179 + open new PR for Phase 2)

## Open coordination

- **PR #179** open with 14+ commits — captain may have reviewed; check captain dispatches first
- **mdpal-app dispatch #579** (my reply) — they may dispatch back with confirmation
- **mdpal-app status:** Phase 1B complete, RealCLIService can switch from FakeProcessRunner to real binary now
...
Conventions established + tested:
...
- @ArgumentParser.Flag disambiguated from MarkdownPalEngine.Flag
...
- H7 byte-equal round-trip — fold into 2.5 hardening

## Key Artifacts

- PVR: `usr/jordan/mdpal/pvr-mdpal-20260403-1447.md`
- A&D: `usr/jordan/mdpal/ad-mdpal-20260404.md`
- Plan: `usr/jordan/mdpal/plan-mdpal-20260406.md` (Phase 2 marked complete through 2.3)
...
- Dispatch monitor: was running (task `b0mwsu3oj`). After compact, restart via `/monitor-dispatches` if not auto-resumed.
...
Re-read this handoff. Restart dispatch monitor if needed. Then PROCEED IMMEDIATELY to Iter 2.4 — no need to ask Jordan. Start with engine Diff API since it gates the `mdpal diff` command. Then ship the 7 remaining commands. Then iteration-complete.
