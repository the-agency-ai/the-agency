...
The handoff tool writes to `usr/jordan/mdpal-cli/mdpal-cli-handoff.md` (agent scope, not workstream scope). An old copy exists at `usr/jordan/mdpal/mdpal-cli-handoff.md` — both were updated this session for safety. Canonical is the `mdpal-cli/` path.
