...
setup() {
    ...
    TOOL="$REPO_ROOT/claude/tools/agency-sweep"
...
@test "dry-run is default; no files modified" {
    echo "use claude/tools/foo" > test.md
    git add test.md; git commit -q -m seed
    run "$TOOL" --manifest manifest.yaml --allowlist allowlist.txt
    [ "$status" -eq 0 ]
    [[ "$output" == *"WOULD-CHANGE"* ]] || [[ "$(cat test.md)" == *"claude/tools/foo"* ]]
    # File content unchanged
    [ "$(cat test.md)" = "use claude/tools/foo" ]
}

@test "--apply rewrites claude/tools/ → agency/tools/" {
    echo "use claude/tools/foo" > test.md
    git add test.md; git commit -q -m seed
...
@test "multi-substitution on one line (manifest-ordered)" {
    echo "claude/tools/x claude/hooks/y" > test.md
    git add test.md; git commit -q -m seed
    run "$TOOL" --manifest manifest.yaml --allowlist allowlist.txt --apply
    [ "$status" -eq 0 ]
...
allowed_substitutions:
  - pattern: "claude/tools"
    replacement: "WON"
  - pattern: "claude/tools/x"
    replacement: "LOSS"
EOF
    echo "claude/tools/x" > test.md
    git add test.md; git commit -q -m seed
...
@test "--files list bypasses ls-files discovery" {
    echo "claude/tools/a" > a.md
    echo "claude/tools/b" > b.md
...
@test "binary files are skipped (no corruption)" {
    ...
    [ "$(od -An -c bin.dat | tr -d ' \n')" = "$(printf '\x00\x01claude/tools/\x02' | od -An -c | tr -d ' \n')" ] || true
}
