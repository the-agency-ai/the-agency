#!/usr/bin/env bats
#
# Tests for tools/tool-new --provider flag
#
...
@test "tool-new --provider=secrets: creates secret-{name}" {
    run_tool tool-new --provider=secrets test-s1 "Test secrets provider"
    assert_success
    assert_file_exists "${TOOLS_DIR}/secret-test-s1"
    rm -f "${TOOLS_DIR}/secret-test-s1"
}

@test "tool-new --provider=secrets: template has correct tool name" {
    run_tool tool-new --provider=secrets test-s2 "Test secrets provider"
    assert_success
    assert_file_contains "${TOOLS_DIR}/secret-test-s2" "secret-test-s2"
    rm -f "${TOOLS_DIR}/secret-test-s2"
}

@test "tool-new --provider=secrets: template has correct dispatcher" {
    run_tool tool-new --provider=secrets test-s3 "Test secrets provider"
    assert_success
    assert_file_contains "${TOOLS_DIR}/secret-test-s3" "Dispatched via: ./tools/secret"
...
@test "tool-new --provider=terminal: creates terminal-setup-{name}" {
    run_tool tool-new --provider=terminal test-t1 "Test terminal provider"
    assert_success
...
@test "tool-new --provider=platform: creates platform-setup-{name}" {
    run_tool tool-new --provider=platform test-p1 "Test platform provider"
    assert_success
...
@test "tool-new --provider=design: creates design-{name}" {
    run_tool tool-new --provider=design test-d1 "Test design provider"
    assert_success
...
@test "tool-new --provider: created tool is executable" {
    run_tool tool-new --provider=secrets test-exec "Test"
    assert_success
    [[ -x "${TOOLS_DIR}/secret-test-exec" ]]
    rm -f "${TOOLS_DIR}/secret-test-exec"
}

@test "tool-new --provider: created tool has provider pattern in content" {
    run_tool tool-new --provider=secrets test-content "Test"
    assert_success
    assert_file_contains "${TOOLS_DIR}/secret-test-content" "secrets"
    rm -f "${TOOLS_DIR}/secret-test-content"
}

@test "tool-new --provider: created tool has set -euo pipefail" {
    run_tool tool-new --provider=secrets test-strict "Test"
    assert_success
    assert_file_contains "${TOOLS_DIR}/secret-test-strict" "set -euo pipefail"
    rm -f "${TOOLS_DIR}/secret-test-strict"
}

@test "tool-new --provider: created tool --help works" {
    run_tool tool-new --provider=secrets test-help "Test help provider"
    assert_success
...
@test "tool-new --provider=invalid: rejects unknown pattern" {
    run_tool tool-new --provider=invalid test-bad "Test"
    assert_failure
    assert_output_contains "Unknown provider pattern"
}

@test "tool-new --provider: rejects if remapped tool already exists" {
    # secret-vault already exists
    run_tool tool-new --provider=secrets vault "Test"
    assert_failure
