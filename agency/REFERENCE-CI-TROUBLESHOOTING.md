...
GH_TOKEN=$(./agency/tools/secret get github-admin-token) gh run list --limit 10

# View specific run logs
GH_TOKEN=$(./agency/tools/secret get github-admin-token) gh run view RUN_ID --log

# View just the failures
GH_TOKEN=$(./agency/tools/secret get github-admin-token) gh run view RUN_ID --log 2>&1 | grep -A 20 "fail)"
```
...
GH_TOKEN=$(./agency/tools/secret get github-admin-token) gh run list --limit 5

# Get detailed failure output
GH_TOKEN=$(./agency/tools/secret get github-admin-token) gh run view RUN_ID --log 2>&1 | tail -150
```
...
3. Commit and push
4. Monitor CI: `GH_TOKEN=$(./agency/tools/secret get github-admin-token) gh run list --limit 1`
...
GH_TOKEN=$(./agency/tools/secret get github-admin-token) gh run view RUN_ID --log 2>&1 | grep -B 5 -A 10 "Expected:"

# Search for specific test
GH_TOKEN=$(./agency/tools/secret get github-admin-token) gh run view RUN_ID --log 2>&1 | grep -A 20 "test name here"
```
