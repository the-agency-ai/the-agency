...
Claude Code fires hooks at well-defined lifecycle events. TheAgency uses these to inject context, check messages, validate state, and run telemetry. Hooks live in `agency/hooks/` and are wired in `.claude/settings.json`.
...
**Important:** `$CLAUDE_PROJECT_DIR` is **only set inside hook execution**, not in agent Bash tool calls. Agents must use `./agency/tools/` (relative paths) when invoking tools — never `$CLAUDE_PROJECT_DIR/agency/tools/...` from a Bash command.

## Hookify Rules

Hookify rules are `PreToolUse` hooks with pattern matching that block or warn when an agent is about to do something that violates conventions. They live in `agency/hookify/hookify.{name}.md` and are activated by symlinking into `.claude/hookify.{name}.local.md`.
...
Sandboxed: remove the symlink from `.claude/hookify.{name}.local.md`. The rule file stays in `agency/hookify/` but isn't active.

Removing entirely: delete from both `agency/hookify/` and `.claude/`. Don't do this without principal approval.
