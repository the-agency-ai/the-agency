...
### 1. New tool — `agency/tools/agent-context`
...
1. `./agency/tools/agent-context` — load your principal-scoped operating context
2. `./agency/tools/handoff read` — your current handoff
3. Check ISCP: `./agency/tools/dispatch list` and `./agency/tools/flag list`
4. Check cross-repo: `./agency/tools/collaboration check`
5. Follow the "Next Action" in your handoff. Do not wait for a prompt.

**Tool usage:** All Agency tools work from ANY directory. Never prefix with `cd`. Use `./agency/tools/`.
```

mdpal-app / mdpal-cli variants: add a sibling step invoking `./agency/tools/handoff read --agent {counterpart}` if the flag exists, else read the runtime-resolved counterpart path.
...
### 3. Fix `agency/tools/lib/_health-agent:82`
...
### 4. Fix `agency/tools/commit-precheck:491`
...
- `agency/config/manifest.json` → `agency_version: 41.19`, `updated_at`
...
- `/Users/jdm/code/the-agency/agency/tools/agent-context` (new)
...
- `/Users/jdm/code/the-agency/agency/tools/lib/_health-agent` (lines 81–98)
- `/Users/jdm/code/the-agency/agency/tools/commit-precheck` (line 491)
...
- `agency/tools/lib/_path-resolve` — sets `AGENCY_PRINCIPAL`, `AGENCY_PRINCIPAL_DIR`, `AGENCY_PROJECT_ROOT`; resolves via `agency.yaml` → `$USER`
- `agency/tools/agent-identity` — `--agent`, `--principal`, `--json`; branch-keyed cache
- `agency/tools/handoff` — `read` already resolves via `_path-resolve` + `agent-identity`
- Multi-principal iteration pattern from `agency/tools/handoff:118` and `agency/tools/instruction-show:90`
...
- Any changes to `agency/agents/*/agent.md` class docs — already principal-agnostic
...
2. Write `agency/tools/agent-context` (new)
