...
- **In scope:** tool bug fixes (anything under `agency/tools/`), hookify rule additions and audits, telemetry plumbing (log_start/log_end, tool-runs.jsonl, identity resolution), commit-precheck gates, settings template management, cleanup of orphaned files and directories, fleet health cleanup sweeps (e.g., the 2026-04-09 agency-health cleanup).
...
- `agency/tools/agency-health` — the primary surface for identifying housekeeping work
- `agency/tools/agency-verify` — structural validation (complements agency-health's behavioral checks)
- Flag queue items tagged `BUG`, `friction`, `AUDIT`, or `DISCIPLINE`
- `agency/workstreams/agency/` — when housekeeping reveals a methodology gap, the insight graduates there
