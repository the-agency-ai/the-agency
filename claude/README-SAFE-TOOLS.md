...
The Agency's safe tools are wrapped versions of shell commands that hookify blocks in their raw form. Raw `git commit`, `cp`, and `gh pr create` are blocked because they bypass the framework's safety checks, commit conventions, and worktree discipline. Safe tools are the approved path — they enforce the rules at the tool layer so hookify can trust them.
...
### `git-safe` — Read + Merge Ops (all agents)

Safe read-only and lightly-guarded write operations for any agent. Subcommands: `status`, `log`, `diff`, `branch`, `show`, `blame`, `add`, `merge-from-master`.

The `add` subcommand blocks `-A`, `--all`, `.`, directories, and glob wildcards — it requires explicit file paths. `merge-from-master` auto-detects `main` vs `master`, refuses on a dirty tree, and merges with `--no-edit`.

### `git-captain` — Privileged Ops (captain only)

Captain-level operations: `merge-to-master`, `checkout-branch`, `switch-branch`, `push`, `fetch`, `tag`, `branch-delete`.

Guards include: merge-to-master verifies you are on main before merging; push blocks main/master targets and bare `--force` (requires `--force-with-lease`); checkout-branch validates branch naming conventions; branch-delete uses `-d` (not `-D`) so unmerged branches are protected.

### `git-safe-commit` — QG-Aware Commit Wrapper

Structured commit formatting with work item tracking. Requires `--work-item` (e.g., `REQUEST-jordan-0065`) and `--stage` (impl, review, tests), or `--no-work-item` as an explicit escape hatch. Builds standardized commit messages, adds per-agent attribution trailers, dispatches a commit notification to captain. Use via the `/git-safe-commit` skill or `/iteration-complete`.

### `git-push` — Push to Origin

Thin push wrapper that blocks pushes to `main`/`master`. Accepts `--force-with-lease` for PR branch updates. Defaults to the current branch. Used by `/sync` and `/ship`.
...
2. A valid receipt must exist in `claude/receipts/` and pass `receipt-verify`
...
Release walkthrough using these tools end-to-end: `claude/YOUR-FIRST-RELEASE.md`
