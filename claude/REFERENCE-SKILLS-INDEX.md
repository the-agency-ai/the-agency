...
|---|---|---|
| `2` | 4 | Full v2 methodology — shipped as case studies 2026-04-19 |
| `1` | ~60 | Legacy skills awaiting v2 upgrade |

**Retrofit progress:** `4 / 64 ≈ 6.25%` at v2. Target: all framework-level active skills at v2 before end of the-agency#298 rollout.
...
| post-merge | Post-PR-merge flow — verify merge, sync master, invoke /sync-all, cut release | 1 | captain | active | GIT-MERGE-NOT-REBASE, WORKTREE-DISCIPLINE |
...
| pr-merge | Merge PR safely — true merge commit (never squash/rebase), --principal-approved gate | 1 | captain | active | GIT-MERGE-NOT-REBASE |
...
| release | Quality-check, commit, push, create PR, cut release in one flow | 1 | captain | active | QUALITY-GATE, CODE-REVIEW-LIFECYCLE |
...
| sync | Merge target into current branch + push to origin (the ONLY command that pushes) | 1 | both | active | GIT-MERGE-NOT-REBASE |
| sync-all | Fetch, merge origin into master, merge worktree work, sync all worktrees. NEVER pushes. | 1 | captain | active | GIT-MERGE-NOT-REBASE, WORKTREE-DISCIPLINE |
...
- [ ] `pr-merge` → **`pr-captain-merge`** (Case study #3 — in progress 2026-04-19)
- [ ] `pr-create` → **`pr-captain-create`**
- [ ] `post-merge` → **`pr-captain-post-merge`** (or keep `post-merge` if it remains generic)
