...
Run after a PR is merged on GitHub. Verifies the merge, merges origin into master, then invokes `/sync-all`. **Never resets master to origin.** See `agency/REFERENCE-GIT-MERGE-NOT-REBASE.md`.
...
**Never `git reset --hard origin/master`.** See `agency/REFERENCE-GIT-MERGE-NOT-REBASE.md`.
