...
Structured discussion tool using the 1B1 (one-by-one) protocol. Each item gets its own resolution cycle.
...
2. **Present the item list.** Show all items numbered, with the first one highlighted as active:
...
3. **For each item, follow the 8-step resolution cycle:**
...
4. **On completion.** After all items resolved, present a decisions summary:
...
- **If the principal says "skip" or "park",** mark the item as skipped/parked and move to the next one.
